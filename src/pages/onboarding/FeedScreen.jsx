    ...
    <div className="w-full h-full bg-[#F7F7F4] flex flex-col justify-between pt-[64px] pb-10 px-6 select-none overflow-y-auto scrollbar-none">
      ...
      <div>
        <div className="flex items-center justify-between py-2 border-b border-[#E5E7EB]">
          ...
          <div className="w-8 h-8 rounded-full bg-[#2B2437]/5 flex items-center justify-center">
            <Notification size={18} color="#2B2437" variant="Linear" />
          </div>
        </div>
        ...
        <div className="mt-5 p-4 bg-white rounded-2xl border border-[#E5E7EB] shadow-card-soft">
          ...
          <h2 className="text-lg font-bold text-[#2B2437]">
        ...
        <div className="mt-4 space-y-3">
          <h3 className="text-xs font-bold text-[#2B2437] uppercase tracking-wider px-1">
          ...
          <div className="bg-white rounded-xl p-3 border border-[#E5E7EB] flex items-center justify-between text-xs">
            <div className="flex items-center gap-2">
              <Call size={16} color="#2B2437" variant="Linear" />
              <span className="font-semibold text-[#2B2437]">
            ...
            <div className="flex flex-wrap gap-2">
              {selectedStates.length > 0 ? (
                selectedStates.map((st) => (
                  <span
                    key={st.id}
                    className="inline-flex items-center gap-1.5 bg-[#2B2437]/5 border border-[#2B2437]/15 px-3 py-1 rounded-full text-xs font-medium text-[#2B2437]"
                  >
                    <span className="w-4 h-4 rounded-full bg-[#F5B55C] text-[#2B2437] flex items-center justify-center text-[10px] font-bold">
                      {st.priority}
                    </span>
                    <span>{st.name}</span>
                  </span>
          ...
          {/* Cities Selected */}
          <div className="bg-white rounded-xl p-3 border border-[#E5E7EB]">
            <div className="text-xs font-bold text-[#2B2437] mb-2">
      ...
      <div className="mt-6">
        <button
          ...
          className="w-full h-[56px] rounded-[16px] bg-[#2B2437] text-white font-bold text-[15px] shadow-md active:scale-[0.99] flex items-center justify-center gap-2 cursor-pointer hover:bg-[#3D334E] transition-colors"
        >
