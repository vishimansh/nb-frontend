 ...
 * State label: centered below image, text-[16px] font-bold #2B2437.
 */
export default function StateCard({ state, isSelected, priority, onClick }) {
  return (
    ...
    >
      ...
      <div className="relative">
        ...
        <div 
          className={`w-[148px] h-[148px] rounded-[28px] overflow-hidden relative bg-[#E5E7EB] border transition-all duration-200 ${
            isSelected 
              ? 'border-[#2B2437] shadow-md ring-2 ring-[#2B2437]/20 scale-[1.01]' 
        ...
        {/* Priority Badge: 32px x 32px numbering icon overlapping the top right corner */}
        {isSelected && priority && (
          <div className="absolute -top-2 -right-2 w-[32px] h-[32px] rounded-full bg-[#2B2437] text-white border-2 border-white flex items-center justify-center text-[16px] font-bold leading-none select-none shadow-md animate-in fade-in zoom-in duration-150 z-20">
      ...
      <span className="mt-2 text-[20px] font-medium text-[#2B2437] text-center tracking-tight leading-tight">
        {state.name}
      </span>
    </div>
  );
