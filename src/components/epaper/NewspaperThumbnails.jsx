...
export function NavaBharatThumbnail({ cityName = 'भोपाल', dateText = '28 अगस्त' }) {
  return (
    <div className="w-full h-full bg-[#FDFCF7] text-[#2B2437] flex flex-col justify-between p-1.5 select-none overflow-hidden text-left relative font-serif">
      {/* Top Paper Header Strip */}
      <div className="border-b border-[#2B2437]/20 pb-0.5 shrink-0">
        <div className="flex items-center justify-between text-[6px] font-sans text-[#6B7280] font-medium tracking-tight leading-none">
          ...
          <span className="font-bold text-[#F5B55C]">{cityName}</span>
          <span>₹ 4.50</span>
        </div>
      ...
      <div className="pt-0.5 mt-0.5 border-t border-[#2B2437]/20 flex items-center justify-between text-[5px] text-[#94A3B8] font-sans shrink-0 leading-none">
        <div className="flex items-center gap-1">
          <div className="w-8 h-1.5 bg-gradient-to-r from-[#2B2437] via-[#475569] to-[#2B2437] opacity-60 rounded-[1px]" />
          <span>ISSN 0971-8257</span>
        </div>
        <span className="font-semibold text-[#2B2437]">Page 01</span>
      </div>
    </div>
  );
}
...
export function SuruchiThumbnail({ dateText = '28 अगस्त' }) {
  return (
    <div className="w-full h-full bg-gradient-to-br from-[#FAF5FF] via-[#F3E8FF] to-[#EDE9FE] text-[#2B2437] flex flex-col justify-between p-1.5 select-none overflow-hidden text-left relative font-sans">
      {/* Top Magazine Header */}
